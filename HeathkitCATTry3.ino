...
// First change:  create some variables
int receivedserial[6];  
byte ACK = 0;

/**
   Raduino_v1.24 for BITX40 - Allard Munters PE1NWL (pe1nwl@gooddx.net)
   With GLG changes
   Frequencies set for the BreadPan Raduino
   25MHZ actually 25.004 460  MHZ
   Heathkit crystal frequencies for the EOC computer 
...
// ***  HAM BAND PARAMETERS ***
#define   LOW80     3300000UL
#define   HIGH80    4050000UL
#define   LOW40       6950000UL
#define   HIGH40      7350000UL
#define   LOWCHUWWV      14650000UL
#define   HIGHCHUWWV     15200000UL
#define   LOW20       13950000UL
#define   HIGH20      14400000UL
#define   LOW15       20950000UL
#define   HIGH15      21500000UL
#define   LOW10       27950000UL
#define   HIGH101     28550000UL

// ***  HEATHKIT PARAMETERS ***
#define   HET80M   12392590UL
#define   HET40M   15888760UL
#define   HET20M   22893940UL
#define   HET15M   29894430UL
#define   HET10M1  36895000UL
#define   BFOLSB  3393630UL
#define   BFOUSB  3396460UL
#define   BFOCW   3395400UL

//  USE 20 METER BANDSWITCH TO TRY TO RECEIVE WWV AND 30 METER BAND


/**
BASELINE HEATHKIT FREQUENCIES FOR REFERENCE
80MeterHeterodyne   12395000UL
40MeterHeterodyne   15895000UL
20MeterHeterodyne   22895000UL
15MeterHeterodyne   29895000UL
10MPartOneHeterodyne  36895000UL
BFOLSB  3393600UL
BFOUSB  3396400UL
BFOCW   3395400UL
...
#define MIN_FREQ 3500000UL        // absolute minimum tuning frequency in Hz
#define MAX_FREQ 28500000UL        // absolute maximum tuning frequency in Hz
...
#define SI5351BX_XTAL 25004460          // Crystal freq in Hz
...
char c[17], b[11], printBuff[2][17];   //Added 1 digit to b to allow for frequencies > 10 MHz
...
void updateDisplay() {
  ...
  if (locked || RUNmode == RUN_FINETUNING) {
    ultoa((frequency + fine), b, DEC);   // fill b string with the frequency 
    strcpy(c, "");
    c[0] = b[0];
  }
  else {        // NOT FINE TUNING MODE
    ultoa((frequency + 50), b, DEC);
    if (!vfoActive) // VFO A is active
      strcpy(c, "A");
    else
      strcpy(c, "B");
    if(frequency>=10000000UL) {
        c[1]=b[0];  // since above 10MHz, get tens digit MHz
        c[2]=b[1];  // since above 10MHz, get ones digit MHz
    }
    else  {
        strcat(c," ");  // add a space
        c[2] = b[0];    // put in the MHz digit when under 10 MHz
    }
      
    }
    
  // If we are above 10MHz we've pulled 2 characters; if below, we've pulled only one. 

  strcat(c, ".");    //Add a decimal point

  if(frequency>10000000UL) strncat (c, &b[2], 3);  // get the kiloherz characters
  else strncat (c, &b[1], 3);   // get kHz characters if below 10 MHz
  
  strcat(c, ".");     // Add a decimal before the Hz
  if (locked || RUNmode == RUN_FINETUNING)
    if(frequency>10000000UL) strncat(c, &b[5], 3);  // show two more digits
    else strncat(c, &b[4], 3); // show two more digits
  else
    if(frequency>10000000UL) strncat(c, &b[5],1);   // add the hundreds of Hz digit
    else strncat(c, &b[4], 1); // add the hundreds of Hz digit
...
/**
#define   LOW40       7000000UL
#define   HIGH40      7300000UL
#define   LOW20       14000000UL
#define   HIGH20      14350000UL
#define   LOW15       21000000UL
#define   HIGH15      21450000UL
#define   LOW10       28000000UL
#define   HIGH101     28500000UL

// ***  HEATHKIT PARAMETERS ***
#define   HET80M   12395000UL
#define   HET40M   15895000UL
#define   HET20M   22895000UL
#define   HET15M   29895000UL
#define   HET10M1  36895000UL
#define   BFOLSB  3393600UL
#define   BFOUSB  3396400UL
#define   BFOCW   3395400UL
**/

unsigned long heterodynefrequency; 


heterodynefrequency=BFOLSB+f;  // Set it to a reasonable value for safety at the beginning

if ( (f>LOW80) && (f<HIGH80)) heterodynefrequency = HET80M;
if ( (f>LOW40) && (f<HIGH40)) heterodynefrequency = HET40M;
if ( (f>LOW20) && (f<HIGH20)) heterodynefrequency = HET20M;
if ( (f>LOWCHUWWV) && (f<HIGHCHUWWV)) heterodynefrequency = HET20M;  // use 20 meter bandswitch for CHU and WWV  meters
if ( (f>LOW15) && (f<HIGH15)) heterodynefrequency = HET15M;
if ( (f>LOW10) && (f<HIGH101)) heterodynefrequency = HET10M1;


  if (mode & 1) // if we are in UPPER side band mode
    si5351bx_setfreq(2, ( heterodynefrequency - BFOUSB - f - RXshift + RIT + fine));

// frequency we are ON is equal to  Heterodyne frequency - BFOfrequency - VFO frequency
// example:                  7.0 MHz (usb) =  15.895 - 3.3964 -  5.4986 (VFO)
//                           so.... VFO frequency = Heterodyne frequency  - BFO frequency - desired frequency
    
  else // if we are in LOWER side band mode
    si5351bx_setfreq(2, (heterodynefrequency - BFOLSB - f  - RXshift - RIT - fine));
  updateDisplay();
}
...
}  // END OF KEYER SUBROUTINE


/**
   The Function Button is used for several functions
   NORMAL menu (normal operation):
   1 short press: swap VFO A/B    ---->   switch Upper and Lower Sideband  (previous 4)
   2 short presses: toggle RIT on/off   ---->  swap VFO A/B  (previous 1)
   3 short presses: toggle SPLIT on/off
   4 short presses: toggle LSB/USB  ------>  toggle RIT on/off (previous 2) 
...
void checkButton() {
  ...
  switch (action) {
    ...
    case 2: // swap the VFOs
      swapVFOs();
      EEPROM.put(26, vfoActive);
      delay(700);
      break;

    case 4: // toggle the RIT on/off
      toggleRIT();
      delay(700);
      break;
    ...
    case 1: // toggle the mode LSB/USB
      toggleMode();
      delay(700);
      break;
  ...
  if (knob < 3) {
    if (frequency > LOWEST_FREQ) {
      if (POT_SPAN < 25)
        ...
        baseTune = baseTune - 50000UL; // mod fast tune down in 50 kHz steps
      frequency = baseTune + (unsigned long)knob * (unsigned long)POT_SPAN / 10UL;
      if (clicks < 10)
        printLine(1, (char *)"<<<<<<<"); // tks Paul KC8WBK


// Move SLOWLYthrough the ham bands, and QUICKLY between them....
      if (
       ((frequency>LOW80) && (frequency<HIGH80) ) ||
       ((frequency>LOW40) && (frequency<HIGH40) ) ||
       ((frequency>LOWCHUWWV) && (frequency<HIGHCHUWWV) ) ||
       ((frequency>LOW20) && (frequency<HIGH20) ) ||
       ((frequency>LOW15) && (frequency<HIGH15) ) ||
       ((frequency>LOW10) && (frequency<HIGH101) ) 
      ) delay(1000);
       delay(10);  // MOVE QUICK BETWEEN BANDS
  ...
  // if the POT_SPAN is very small (less than 25 kHz) then use 1 kHz steps instead
    ...
    if (frequency < HIGHEST_FREQ) {
      if (POT_SPAN < 25)
        ...
        baseTune = baseTune + 50000UL; // mod fast tune up in 50 kHz steps
      frequency = baseTune + (unsigned long)knob * (unsigned long)POT_SPAN / 10UL;
      if (clicks < 10)
        printLine(1, (char *)"         >>>>>>>"); // tks Paul KC8WBK        
...
void setup() {
  raduino_version = 24;
  strcpy (c, "Raduino 1.24KX4Z");

  lcd.begin(16, 2);

  // Start serial and initialize the Si5351
  Serial.begin(9600);
    printLine(1, (char *)"SerialInit");
  delay(1000);
